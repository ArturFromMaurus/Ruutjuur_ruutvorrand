...
function Ylesanne(){
  ...
  if (mudeli_valik==2){
      ...
      while (true){
        ...
        a=1;
        while (c==0  || ((-c)/a)<0 || a==0 ){
          a=int(random_int(100));
          while (a==0){
             a=int(random_int(100));
          }
          c=int(random_int(100))
        }
        X1=-1*Math.sqrt(-c/a);
        X2=1*Math.sqrt(-c/a); // ############################################## UURI KUIDAS MURRULISTE VASTUSTE PUHUL TOIME TULLA
        
        if ( (Number.isInteger(X1) && Number.isInteger(X2))  || ((str(X2).split(".")[1]).length<5  && (str(X1).split(".")[1]).length<5)   ){
          console.log(X1,X2)
          break;
        }
      }
